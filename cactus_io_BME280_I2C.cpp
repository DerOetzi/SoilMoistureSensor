...
float BME280_I2C::getHumidity(void) {
    float temp_real = temperature + tempcal;
    float temp_exp1 = (float) exp((17.67 * temperature) / (temperature + 243.5));
    float temp_exp2 = (float) exp((17.67 * temp_real) / (temp_real + 243.5));
    return (temp_exp1 * (273,15 + temp_real) * humidity) / (temp_exp2 * (273,15 + temperature));
    //return humidity;
}
